...
@torch.jit.script
def calculate_sse(
    data: torch.Tensor, centers: torch.Tensor, cluster_labels: torch.Tensor
) -> float:
    sse = torch.sum(
        torch.stack(
            [
...
def merge_clusters(clusters, merge_indices):
    ...
    merged_cluster = torch.where(
        clusters == merge_indices[1], merge_indices[0], clusters
    )
    return torch.where(
        merged_cluster > merge_indices[1], merged_cluster - 1, merged_cluster
    )
...
def kmeans_silhouette(features):
    ...
    while k > 2:
        ...
        if len(torch.unique(clusters)) > 1:
            avg_silhouette = silhouette_score(
                features.cpu().numpy(), clusters.cpu().numpy()
            )
    ...
    return (
        best_clusters.cpu().numpy(),
        best_centers.cpu().numpy(),
        best_k,
        center_indices,
    )
