...
class MarianTranslator(BaseTranslation):
    def __init__(self, from_lang=None, to_lang="en", auto_clean=False):
        ...
        try:
            ...
            raise ValueError(
                f"Error loading model {self.model_id}. `transformers` is not installed. Please try `pip install transformers`"
            )
...
class M2M100Translator(BaseTranslation):
    ...
    def run(self, text) -> str:
        ...
        outputs = self.model.generate(
            input_ids, forced_bos_token_id=self.tokenizer.get_lang_id("en")
        )
        result = self.tokenizer.decode(outputs[0], skip_special_tokens=True)
        return result
