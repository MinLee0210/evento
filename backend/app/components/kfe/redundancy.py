...
def filter_low_information_frames(
    histograms: torch.Tensor, keyframe_indices: list[int], threshold: int = 10
) -> tuple[torch.Tensor, list[int]]:
    mask = torch.sum(histograms > 0, dim=1) > threshold
    return histograms[mask], [
        keyframe_indices[i] for i in range(len(keyframe_indices)) if mask[i]
    ]


def remove_redundant_frames(
    similarity_matrix: torch.Tensor, keyframe_indices: list[int], threshold: float
) -> list[int]:
    del_indices = set()
...
def redundancy_(
    video, keyframe_indices: list[int], threshold: float = 0.94
) -> list[int]:
    histograms = compute_color_histograms(video, keyframe_indices)
    filtered_histograms, filtered_indices = filter_low_information_frames(
        histograms, keyframe_indices
    )
    similarity_matrix = calculate_similarity_matrix(filtered_histograms)
    final_indices = remove_redundant_frames(
        similarity_matrix, filtered_indices, threshold
    )
